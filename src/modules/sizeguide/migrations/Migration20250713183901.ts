...
export class Migration20250713183901 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "sizechart-measure" ("id" text not null, "size" text not null, "neck_girth" integer null, "shoulder_width" integer null, "neck_elbow" integer null, "chest_girth" integer null, "back_width" integer null, "hip_girth" integer null, "arm_length" integer null, "arm_girth" integer null, "wrist_girth" integer null, "thigh_girth" integer null, "ankle_girth" integer null, "trunk_height" integer null, "leg_height" integer null, "sizechart_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "sizechart-measure_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_sizechart-measure_sizechart_id" ON "sizechart-measure" (sizechart_id) WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_sizechart-measure_deleted_at" ON "sizechart-measure" (deleted_at) WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "sizechart-relevant-measure" ("id" text not null, "field_name" text not null, "sizechart_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "sizechart-relevant-measure_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_sizechart-relevant-measure_sizechart_id" ON "sizechart-relevant-measure" (sizechart_id) WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_sizechart-relevant-measure_deleted_at" ON "sizechart-relevant-measure" (deleted_at) WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "user-size" ("id" text not null, "neck_girth" integer null, "shoulder_width" integer null, "neck_elbow" integer null, "chest_girth" integer null, "back_width" integer null, "hip_girth" integer null, "arm_length" integer null, "arm_girth" integer null, "wrist_girth" integer null, "thigh_girth" integer null, "ankle_girth" integer null, "trunk_height" integer null, "leg_height" integer null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "user-size_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_user-size_deleted_at" ON "user-size" (deleted_at) WHERE deleted_at IS NULL;`);

    this.addSql(`alter table if exists "sizechart-measure" add constraint "sizechart-measure_sizechart_id_foreign" foreign key ("sizechart_id") references "sizechart" ("id") on update cascade;`);

    this.addSql(`alter table if exists "sizechart-relevant-measure" add constraint "sizechart-relevant-measure_sizechart_id_foreign" foreign key ("sizechart_id") references "sizechart" ("id") on update cascade;`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`drop table if exists "sizechart-measure" cascade;`);

    this.addSql(`drop table if exists "sizechart-relevant-measure" cascade;`);

    this.addSql(`drop table if exists "user-size" cascade;`);
  }
